app.controller("ExchangeController", function ($scope, $rootScope, $cookieStore, $http, $filter) {
    ...
    function PosScnFull() {
        ...
        $http({
            url: '/SaleExchange/GetPOSScreenFull',
            method: "GET",
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
    ...
    function GetAllInvoiceNumber() {
        if ($scope.ddlOutLet) {
            ...
            $http({
                url: '/SaleExchange/GetInvoiceNo?terminalId=' + $scope.pos_Sale.TerminalId + '&saleDate=' + from,
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            }).success(function (data) {
    ...
    function GetAllPaymentType() {
        $http({
            ...
            $scope.PaymentTypeIsAdjustmentList = Enumerable.From(data).Where('$.IsAdjustment == true').ToArray();
    ...
    $scope.BarcodeSearch = function (e) {
        $http({
            url: '/SaleExchange/SaleDetailAdAttributeGetByDepartmentAndBarcode?departmentId=' + $scope.ddlOutLet.DepartmentId + '&barcode=' + $scope.BarCode,
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
    ...
    $scope.SaveSale = function () {
        ...
        else if ($scope.totalExchange > $scope.pos_Sale.NetPayable) {
            erroMsg.push({ msg: "Exchange amount cannot be more then payable amount." });
        }
            /*
        else if ($scope.SaleSaveLbl == 'ReIssue Sale') {
            if ($scope.pos_Sale.ReIssueReason == null || $scope.pos_Sale.ReIssueReason == undefined || $scope.pos_Sale.ReIssueReason == '') {
                erroMsg.push({ msg: "Please Enter ReIssue Reason" });
            }
            if (($scope.PreSaleMaster.PaidAmount - $scope.PreSaleMaster.ChangeAmount) > ($scope.pos_Sale.PaidAmount - $scope.pos_Sale.ChangeAmount)) {
                erroMsg.push({ msg: "Minimum Invoice Amount " + ($scope.PreSaleMaster.PaidAmount - $scope.PreSaleMaster.ChangeAmount) });
            }
        }
            */
        else {
        ...
        if (erroMsg.length > 0) {
            ...
            $scope.pos_Sale.MemoNo = $scope.ddlInvoice.InvoiceNo;
                ...
                if ($scope.CreatePermission) {
                    alertify.confirm("Are you sure to Save?", function (e) {
                        if (e) {
                            ...
                            var Exchange_SaleDetailLst = [];
                            for (var i = 0; i < $scope.Exchange_SaleDetailLst.length; i++) {
                                if ($scope.Exchange_SaleDetailLst[i].Exchenge > 0) {
                                    if (Enumerable.From(SaleDetailLst).Where('$.ItemAddAttId==' + $scope.Exchange_SaleDetailLst[i].ItemAddAttId).ToArray().length > 0) {
                                        alertify.log("Exchange Item and Sale Item Can't be Same!", 'error', '10000');
                                        return;
                                    }

                                    $scope.Exchange_SaleDetailLst[i].Quantity = $scope.Exchange_SaleDetailLst[i].Exchenge * -1;
                                    Exchange_SaleDetailLst.push($scope.Exchange_SaleDetailLst[i]);
                                }

                            }
                            //angular.forEach($scope.Exchange_SaleDetailLst, function (axchange_SaleDetailLst) {
                            //    if (axchange_SaleDetailLst.Exchenge > 0) {
                            //        axchange_SaleDetailLst.Quantity = axchange_SaleDetailLst.Exchenge * -1;
                            //        Exchange_SaleDetailLst.push(axchange_SaleDetailLst);
                            //    }

                            //})
                            $.ajax({
                                url: "/SaleExchange/SaveSale",
                                contentType: "application/json;charset=utf-8",
                                type: "POST",
                                data: JSON.stringify({ _pos_Sale: $scope.pos_Sale, pos_SaleDetailLst: SaleDetailLst, exchange_pos_SaleDetailLst: Exchange_SaleDetailLst, pos_SaleDetailChargeLst: $scope.pos_SaleDetailChargeLst, pos_SaleDetailFreeLst: $scope.pos_SaleDetailFreeLst, pos_SalePaymentLst: $scope.pos_SalePaymentList, nonRegCustomer: $scope.nonRegCustomer, pos_SaleDetailAdAttributeLst: $scope.pos_SaleDetailLst, exchangePos_SaleDetailAdAttributeLst: Exchange_SaleDetailLst }),
                                success: function (data) {
        ...
        if ($scope.SaleInvoiceNo) {
            $http({
                ...
                if ($scope.InvoiceDetails.length > 0) {
                    ...
                    printContents += '<tr><td colspan="4" style="text-align:center;"><strong>*** Thanks for visiting ' + $scope.InvoiceMaster.BranchTypeName + ' ***</strong></td></tr> </tfoot></table></div></div></div></div></div></div>';
    ...
    //    $http({
    //        url: '/SaleExchange/GetReturnDays',
    //        method: "GET",
    //        headers: { 'content-type': 'application/json' }
    ...
    //            theme: 'blue-theme'
    //        });
    //        document.getElementById("txtFromDate").addEventListener("click", function () {
    //            picker1.show();
    //        });
    //        document.getElementById("FromDateCalender1").addEventListener("click", function () {
    //            picker1.show();
    //        });
    //    })
    //}
    ...
    $scope.GetInvoiceNoByDate = function () {
        ...
        var search = "MemoNo='N/A' AND InvoiceNo NOT IN (SELECT MemoNo FROM pos_Sale) AND SaleDate BETWEEN " + "'" + from + "'" + ' AND ' + "'" + to + "'";
        $http({
            url: '/SaleExchange/GetSaleDynamic?searchCriteria=' + search,
            method: 'POST',
            headers: { 'Content-type': 'Application/json' }
        }).success(function (searchdataList) {
            if (searchdataList.length > 0) {
                $scope.InvoiceList = searchdataList;
                alertify.log('Invoices Loaded in Invoice Combo Box.', 'success', '5000');
            }
            else {
                alertify.log(' No Sale invoice for exchange!', 'error', '5000');
            }
        })
    }
    $scope.SaleSearchByInv = function () {
        $scope.Exchange_SaleDetailLst = [];
        var Sale = Enumerable.From($scope.InvoiceList).Where(function (x) { return x.InvoiceNo == $scope.ddlInvoice.InvoiceNo }).ToArray();
        angular.forEach(Sale, function (aSale) {
            $scope.CustomerName = aSale.CustomerName;
            $scope.CustomerMobile = aSale.Mobile;
            $scope.BarCodeCustomer = aSale.CustomerCode;
            $http({
                url: '/SaleExchange/GetSaleDetailById?invoiceNo=' + aSale.InvoiceNo,
                method: 'GET',
                headers: { 'Content-Type': 'application/json' }
            }).success(function (SaleDetail) {
                if (SaleDetail.length > 0) {
                    angular.forEach(SaleDetail, function (aSaleDetail) {
                        if (aSaleDetail.AttributeQtyFree == 0) {
                            $scope.Exchange_SaleDetailLst.push(aSaleDetail);
                        }
                    });
                }
            });
    ...
    $scope.CalculateExchange = function () {
        $scope.totalExchange = 0;
        angular.forEach($scope.Exchange_SaleDetailLst, function (aExchange_SaleDetailLst) {
            if (aExchange_SaleDetailLst.Exchenge == undefined || aExchange_SaleDetailLst.Exchenge == null) { aExchange_SaleDetailLst.Exchenge = 0; }
            if (aExchange_SaleDetailLst.Exchenge > aExchange_SaleDetailLst.Quantity) {
                aExchange_SaleDetailLst.Exchenge = '';
                alertify.log('Exchange quantity must be less then or equal sale quantity.', 'error', '5000');
            }
            else {
                $scope.totalExchange += (aExchange_SaleDetailLst.UnitPrice - aExchange_SaleDetailLst.UnitDiscount) * aExchange_SaleDetailLst.Exchenge;
            }
        })
        if ($scope.pos_SalePaymentList.length > 0) {
            angular.forEach($scope.pos_SalePaymentList, function (a) {
                if (a.PaymentTypeName == 'EXCHANGE CREDIT') {
                    a.Amount = $scope.totalExchange;
                }
            })
        }
        else {
            angular.forEach($scope.PaymentTypeIsAdjustmentList, function (paymentTypeIsAdjustment) {
                if (paymentTypeIsAdjustment.PaymentTypeName == "EXCHANGE CREDIT") {
                    paymentTypeIsAdjustment.Amount = $scope.totalExchange;
                    $scope.pos_SalePaymentList.push(paymentTypeIsAdjustment);
                }
            })
        }
        CalculatTotalCalculation();
