app.controller("SalesOrderEntryController", function ($scope, $cookieStore, $http, $window, $filter) {
    ...
    function SaveOrder(status) {
        ...
        if (!erroMsg.length) {
            ...
            //$scope.salesOrder.CreateDate = '';
            //$scope.salesOrder.UpdateDate = null;
            //if ($scope.salesOrder.CurrentSalesOrderDate != null) {
    ...
    /*function ScreenLock() {
        $http({
            ...
            method: 'GET',
            params: { userId: $scope.LoginUser.UserId, screenId: $cookieStore.get('SalesOrderScreenId') },
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
            if (data != '') {
                ...
                $scope.s_ScreenLock.UserId = $scope.LoginUser.UserId;
                $scope.s_ScreenLock.ScreenId = $cookieStore.get('SalesOrderScreenId');
    ...
    //function GetSalesOrderPaged() {
    //    if ($scope.currentPage == null) $scope.currentPage = 1;
    //    var startRecordNo = ($scope.PerPage * ($scope.currentPage - 1)) + 1;
    //    $http({
    //        url: '/SalesOrder/GetSalesOrderPaged?startRecordNo=' + startRecordNo + '&rowPerPage=' + $scope.PerPage + '&rows=' + 0,
    //        method: 'GET',
    //        headers: { 'Content-Type': 'application/json' }
    //    }).success(function (data) {
    ...
    //        $scope.SalesOrderList = data.ListData;
    ...
    $scope.SaveSalesOrder = function () {
        // console.log($scope.salesOrder.PODate);
        //if ($scope.salesOrder.IsNonSO) {
    ...
    $scope.priceOrAmountChange = function (fromPriceChange) {
        if (fromPriceChange) {
            ...
            // console.log($scope.pos_SalesOrderDetail.Amount);
        }
    }
   ...
   $scope.ItemUnitFilter = function (itemUnit) {
    ...
    function GetAllItem() {
        $http({
            ...
            console.log($scope.ItemSearchList);
        });
    };
    $scope.loadItemDetails = function (aItem) {
        var splitItem = aItem.split("~");
        //$scope.pos_SalesOrderDetail.ItemDescription = splitItem[0].trim() + splitItem[1].trim();
    ...
    function GetAllStorewithDepartment() {
        ...
        $http({
            ...
            console.log('Department', $scope.sockQtyWithRequestQtyList);
            console.log('Department List', $scope.sockQtyWithRequestQty);

        });
    }

    $scope.CheckDdlStock = function (itemId) {
        $scope.itemId = itemId;
       // alert($scope.itemId.ItemAddAttId);
    }
    ...
    $scope.reloadBtn = function () {
        ...
        $scope.FromDate = "";
        $scope.ToDate = "";
        $scope.SearchSalesOrderAndCompanyName = null;
    ...
    function GetSalesOrderPaged(curPage) {
        ...
        if ($scope.SearchSalesOrderAndCompanyName != undefined && $scope.SearchSalesOrderAndCompanyName != "" && $scope.FromDate != "" && $scope.ToDate != "") {
            SearchCriteria = "([SO].[SalesOrderDate] between '" + $scope.FromDate + "' and '" + $scope.ToDate + "') and ([SO].[SalesOrderNo] LIKE '%" + $scope.SearchSalesOrderAndCompanyName + "%' OR [SO].[CompanyNameOnBill] LIKE '%" + $scope.SearchSalesOrderAndCompanyName + "%')";
            ...
            SearchCriteria = "[SO].[SalesOrderNo] LIKE '%" + $scope.SearchSalesOrderAndCompanyName + "%' OR [SO].[CompanyNameOnBill] LIKE '%" + $scope.SearchSalesOrderAndCompanyName + "%'";
            //alert("Name Success!!!!!");
        }
        else if ($scope.FromDate != "" && $scope.ToDate != "") {
            SearchCriteria = "[SO].[SalesOrderDate] between '" + $scope.FromDate + "' and '" + $scope.ToDate + "'";
            //alert("Date Success!!!!!");
        }
        ...
        $http({
            url: encodeURI('/SalesOrder/GetSalesOrderPaged?startRecordNo=' + startRecordNo + '&rowPerPage=' + $scope.PerPage + "&whereClause=" + SearchCriteria + '&rows=' + 0),
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success(function (data) {
    ...
    //$scope.SelSalesOrder = function (aSalesOrder) {
    //    $scope.salesOrder = {};
    //    $scope.salesOrder = aSalesOrder;
    //    $scope.company.CompanyName = aSalesOrder.CompanyName;
